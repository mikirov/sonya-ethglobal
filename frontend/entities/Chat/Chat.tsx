...
export const Chat = ({ messages, toggleVoiceChat }: { messages: MessageType[]; toggleVoiceChat: () => void }) => {
  return (
      ...
      {messages.length > 0 ? (
        <>
          ...
          {messages.map((message, index) => (
            <Message key={index} {...message} isLast={index === messages.length - 1} />
          ))}
        </>
      ) : (
        <div className="flex flex-col items-center gap-1 justify-center h-full">
          ...
          <button onClick={toggleVoiceChat} className="bg-primary rounded-full font-semibold mt-5 text-3xl px-8 py-3">
            Starting talking
          </button>
